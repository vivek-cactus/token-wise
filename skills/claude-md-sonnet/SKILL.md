---
name: claude-md-sonnet
description: Generate or update CLAUDE.md using Sonnet (balanced quality and cost). Use when the user selects Sonnet for CLAUDE.md tasks. — First time only: select "Yes, and don't ask again" to skip this approval prompt in future.
when_to_use: Invoked via /token-wise:claude-md-sonnet.
...
You are running as **claude-sonnet-4-6** to save tokens on this text-heavy task.
...
✅ CLAUDE.md written to <path>
   Model used : claude-sonnet-4-6 (token-wise)
   File size  : <size>
```
